...
def Beta(k0):
    h = 0.508
    w1 = 1.08 # Top strip width
    w2 = 0.55  # Bottom strip width
    s = 0.127  # separation between strips
    t = 0.035
    a = 10.0
    b = 5.0
    Dk = 3.66
    ...
    if mpiRank == gmshRank:
        ...
        gmsh.model.occ.addRectangle(0, h, 0, w1/2, t, tag=3)
        gmsh.model.occ.addRectangle(0, h-t-s, 0, w2/2, t, tag=4)
        gmsh.model.occ.cut([(2,2)],[(2,3)], 105, removeObject=True, removeTool=True)
        gmsh.model.occ.cut([(2,1)],[(2,4)], 106, removeObject=True, removeTool=True)
                
        gmsh.model.occ.synchronize()
        ov, ovv = gmsh.model.occ.fragment([(2,105)],[(2,106)], -1)
        gmsh.model.occ.synchronize()
        pt = gmsh.model.getEntities(0)
        print(pt)
        gmsh.model.mesh.setSize(pt, lm)
        ov = gmsh.model.getEntitiesInBoundingBox(np.min([w1, w2])/2-eps, h-t-s-eps, -eps, np.max([w1, w2])/2+eps, h+t+eps, eps)
        print(ov)
        ...
        for bnd in pt:
            ...
            if(np.abs(CoM[1] - h) < eps) or (np.abs(CoM[1] -h -t) < eps) or (np.abs(CoM[1] - h- t/2) < eps):
                Strip1.append(bnd[1])
            elif(np.abs(CoM[1] - h + s + t) < eps) or (np.abs(CoM[1] - h + s) < eps) or (np.abs(CoM[1] - h + s + t/2) < eps):
                Strip2.append(bnd[1])
            elif CoM[0] < eps:
                PMC.append(bnd[1])
            else:
        ...
        gmsh.model.addPhysicalGroup(1, Strip2, 4)
        gmsh.model.setPhysicalName(1, 1, "PEC")
        gmsh.model.setPhysicalName(1, 2, "PMC")
        gmsh.model.setPhysicalName(1, 3, "Strip1")
        gmsh.model.setPhysicalName(1, 4, "Strip2")
        gmsh.model.addPhysicalGroup(2, [106], 1)
        gmsh.model.setPhysicalName(2, 1, "Substrate")
        gmsh.model.addPhysicalGroup(2, [105], 2)
...
#        Vw = basix.ufl.element('DQ', mesh.basix_cell(), 0, shape=(mesh.geometry.dim, ))
        ...
        for i, kz in vals:
            ...
            if np.real(kz) > eps:
                ...
                Curr1.append(mesh.comm.allreduce(fem.assemble_scalar(fem.form(ufl.inner(tau, curl_2d(eth, ezh, kz)) *  ds(3))), op = nMPI.SUM) / (1j * k0 * eta0))
                Curr2.append(mesh.comm.allreduce(fem.assemble_scalar(fem.form(ufl.inner(tau, curl_2d(eth, ezh, kz)) *  ds(4))), op = nMPI.SUM) / (1j * k0 * eta0))
                Pm.append(mesh.comm.allreduce(fem.assemble_scalar(fem.form(ufl.inner(perm_vec(eth), curl_2d(eth, ezh, kz)) * ufl.dx)), op=nMPI.SUM) / (1j * k0 * eta0))
                beta.append(kz)

    return (Curr1, Curr2, Pm, beta)
...
for n in range(11):
    ...
    for i in range(2):
        Im[n][i][0] = 2.0 * I1[i]
        Im[n][i][1] = 2.0 * I2[i]
        Pm[n][i][i] = 2.0 * np.real(P[i])
        Dk_eff[n][i] = (np.real(b[i])/k0[n])**2
