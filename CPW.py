...
def Beta(k0):
    ...
    if mpiRank == gmshRank:
        ...
        gmsh.model.occ.addRectangle(s/2+w, d1+h, 0, a/2, t, tag=4)
        gmsh.model.occ.addRectangle(0, d1+h, 0, s/2, t, tag=5)
        gmsh.model.occ.cut([(2,3)],[(2,4), (2,5)], 103, removeObject=True, removeTool=True)
        ...
        ov = gmsh.model.getEntitiesInBoundingBox(s/2-eps, h+d1-eps, -eps, s/2+w+eps, h+d1+t+eps, eps)
        print(ov)
        ...
        for bnd in pt:
            ...
            if(np.abs(CoM[0] - s/4) < eps) or (np.abs(CoM[0] - s/2) < eps):
                Strip1.append(bnd[1])
...
#        Vw = basix.ufl.element('DQ', mesh.basix_cell(), 0, shape=(mesh.geometry.dim, ))
    ...
    return kz, Pm / np.abs(Curr1)**2 / 2.0
