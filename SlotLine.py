...
def Beta(k0):
    h = 0.254 # substrate thickness
    w = 0.5  # gap width
    t = 0.035 # Metal thickness
    d1 = 1.5 # substrate height
    a = 10.0 # shield width
    b = 5.0  # shield height
    Dk = 3.66  #substrate dielectric permittivity
    eta0 = 377.0
    ...
    if mpiRank == gmshRank:
        ...
        gmsh.model.add("Slotline WG")
        gmsh.model.setCurrent("Slotline WG")

        gmsh.model.occ.addRectangle(0, 0, 0, a/2, d1, tag=1)
        gmsh.model.occ.addRectangle(0, d1, 0, a/2, h, tag=2)
        gmsh.model.occ.addRectangle(0, d1+h, 0, a/2, b-h-d1, tag=3)
        gmsh.model.occ.addRectangle(w/2, d1+h, 0, a/2, t, tag=4)
        gmsh.model.occ.cut([(2,3)],[(2,4)], 103, removeObject=True, removeTool=True)
        
        gmsh.model.occ.synchronize()
        ov, ovv = gmsh.model.occ.fragment([(2,1)],[(2,2)], -1)
        ov, ovv = gmsh.model.occ.fragment([(2,2)],[(2,103)], -1)
        gmsh.model.occ.synchronize()
        pt = gmsh.model.getEntities(0)
        print(pt)
        gmsh.model.mesh.setSize(pt, lm)
        ov = gmsh.model.getEntitiesInBoundingBox(w/2-eps, h+d1-eps, -eps, w/2+eps, h+d1+t+eps, eps)
        print(ov)
        ...
        for bnd in pt:
            ...
            if(np.abs(CoM[1] - d1 - h) < eps) or (np.abs(CoM[1] - d1 -h -t) < eps) or (np.abs(CoM[1] - d1 - h- t/2) < eps):
                Strip1.append(bnd[1])
            else:
                PEC.append(bnd[1])
        ...
        gmsh.model.addPhysicalGroup(2, [1,103], 2)
...
#        gmsh.fltk.run()
    ...
    eigs.setWhichEigenpairs(SLEPc.EPS.Which.TARGET_IMAGINARY) # target real part of eigenvalue
    eigs.setTarget(-k0*k0/2)
    eigs.setDimensions(nev=24) # Number of eigenvalues
    eigs.solve()
    eigs.view()
    eigs.errorView()
...
#        Vw = basix.ufl.element('DQ', mesh.basix_cell(), 0, shape=(mesh.geometry.dim, ))
        ...
        T = fem.functionspace(mesh, ("Lagrange", 1))
        Ezt = fem.Function(T)
        ...
        for i, kz in vals:
            ...
            if kz > eps:
                ...
                with io.XDMFFile(mesh.comm, "Efield_{0}.xdmf".format(i), "w") as xx:
                    xx.write_mesh(mesh)
                    xx.write_function(Et_dg,0)
                with io.XDMFFile(mesh.comm, "Efield_z_{0}.xdmf".format(i), "w") as xx:
                    xx.write_mesh(mesh)
                    xx.write_function(Ezt,0)
                ...
                with io.XDMFFile(mesh.comm, "Hfield_{0}.xdmf".format(i), "w") as xx:
                    xx.write_mesh(mesh)
                    xx.write_function(Et_dg,0)
                Ezt.interpolate(fem.Expression(zcurl_2d(eth), T.element.interpolation_points()))
                with io.XDMFFile(mesh.comm, "Hfield_z_{0}.xdmf".format(i), "w") as xx:
                    xx.write_mesh(mesh)
                    xx.write_function(Ezt,0)
    ...
    return kz, Pm / np.abs(Curr1)**2

Z0 = np.zeros(11, dtype=np.complex128)
kz = np.zeros(11, dtype=np.complex128)
...
if mpiRank == gmshRank:
    ...
    ps = ax[0].plot(k0, (np.real(kz)/k0)**2, 'r-')
    pt = mp.Patch(color='red', label='Dk effective')
    l1 = ax[0].legend(handles=[pt], loc='lower right', shadow=True)
    ax[0].grid = True
    ax[0].set_ylabel('Eff. Dk')
    ax[0].set_xlabel('k0')

    ps = ax[1].plot(k0, np.real(Z0), 'g-')
    pt = mp.Patch(color='green', label='Char. Impedance')
    l1 = ax[1].legend(handles=[pt], loc='lower right', shadow=True)
    ax[1].grid = True
